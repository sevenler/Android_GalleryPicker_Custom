...
public class ImageListUber implements IImageList {
	...
	public ImageListUber(IImageList[] sublist, int sort) {
		...
		mQueue = new PriorityQueue<MergeSlot>(4,
				sort == ImageManager.SORT_ASCENDING ? new AscendingComparator()
	...
	public IImage getImageAt(int index) {
		if (index < 0 || index > getCount()) {
			throw new IndexOutOfBoundsException("index " + index + " out of range max is "
					+ getCount());
		}
	...
	/**
	 * Modify the skip list when an image is deleted by finding the relevant
	 * entry in mSkipList and decrementing the counter. This is simple because
	 * deletion can never cause change the order of images.
	 */
	 ...
	 * each given sub list, there will be one corresponding merge slot. We use
	 * merge-sort-like algorithm to build the merged list. At begining, we put
	 * all the slots in a sorted heap (by timestamp). Each time, we pop the slot
	 * with earliest timestamp out, get the image, and then move the index
	 * forward, and put it back to the heap.
	 */
